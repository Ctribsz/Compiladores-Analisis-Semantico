...
# Literales de String
    _str_0: .asciiz ""
    _str_1: .asciiz "Hola, soy "
    _str_2: .asciiz "Diego"
    _str_3: .asciiz "saludar"
...
Estudiante_saludar:
    ...
    addu $sp, $sp, 8
    jr $ra
    # end_function Estudiante.saludar

_script_start:
        # Reseteando estado de frame para main
    # t10 = new Estudiante
    # Alocando 12 bytes para Estudiante
    li $a0, 12
    jal _alloc
    sw $v0, -4($fp)
    # push 4
    ...
    # t11 = t10."saludar"
    lw $t0, -4($fp)
    # (Resolviendo dirección de método Estudiante_saludar)
    la $t0, Estudiante_saludar
    sw $t0, -8($fp)
    # push t10
    lw $t0, -4($fp)
    subu $sp, $sp, 4
    sw $t0, 0($sp)
    # t12 = call t11, 1
    # Llamada indirecta a puntero en temporal 't11'
    lw $t0, -8($fp)
    jalr $t0
    sw $v0, -12($fp)
    # SP = SP + 4
